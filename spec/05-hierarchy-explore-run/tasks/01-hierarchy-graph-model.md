# Hierarchy Graph Model

## Assigned Model

- Task default: `gpt-5.3-codex`

## Owning Specs

- `spec/05-hierarchy-explore-run/SPEC.md`
- `docs/TASK-HARNESS.md`

## Goal

Implement tree and directed graph hierarchy modeling for groups, tools, and next steps.

## Scope

- Graph nodes, edges, parent relationships, and availability metadata.
...
| --- | --- | --- |
| Plan | gpt-5.4 | Implementation notes, file ownership, dependencies, and risk check. |
| Use Cases | Inherit task default | Acceptance scenarios and edge cases derived from the owning spec. |
| Test / TDD | Inherit task default | Failing or updated tests before implementation where practical. |
| Develop | gpt-5.4 | Scoped implementation that satisfies the task requirements. |
| Validate | gpt-5.4-mini | Commands run, results recorded, and done criteria checked. |
...
- Include all edge cases from `spec/05-hierarchy-explore-run/SPEC.md` that apply to this task.
...
- Add or update the tests listed in `spec/05-hierarchy-explore-run/SPEC.md` that apply to this task.
